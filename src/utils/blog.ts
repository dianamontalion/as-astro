...
export const findFeaturedPosts = async ({ count }: { count?: number }): Promise<Array<Post>> => {
  const _count = count || 4;
  const posts = await fetchPosts();
  const hotPosts = posts.filter((post) => post.category?.slug === "featured")
  return hotPosts ? hotPosts.slice(0, _count) : [];
};
...
export const getStaticPathsBlogCategory = async ({ paginate }: { paginate: PaginateFunction }) => {
  ...
  const categories: { [key: string]: Category | undefined } = {};;
  posts.map((post) => {
...
export const getStaticPathsBlogTag = async ({ paginate }: { paginate: PaginateFunction }) => {
  ...
  const tags: { [key: string]: Tag | undefined } = {};
  posts.map((post) => {
